...
class VisionTransformer(nn.Cell):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.head = nn.Dense(in_channels=embed_dim, out_channels=num_classes) if num_classes > 0 else Identity()
        ...
        if isinstance(self.head, nn.Dense):
            self.head.weight.set_data(weight_init.initializer(weight_init.TruncatedNormal(sigma=0.02),
        ...
        if isinstance(self.head, nn.Dense):
            P.Assign()(self.head.weight.data, self.head.weight.data * init_scale)
            P.Assign()(self.head.bias.data, self.head.bias.data * init_scale)

    def fix_init_weight(self):
        def rescale(param, layer_id):
            P.Assign()(param, param / math.sqrt(2.0 * layer_id))
...
if __name__ == "__main__":
    ...
    print(f"parameters_and_names:{params}")
    print(index)
    params = 0.
    index = 0
